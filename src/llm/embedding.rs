...
impl Embedder {
    ...
    pub fn embedding_dim(&self) -> usize {
        match &self.backend {
            ...
            EmbedderBackend::Ollama(remote) => remote.dimensions,
        }
    }
    ...
    pub fn embed_query(&self, query: &str) -> Result<Vec<f32>> {
        ...
        match &self.backend {
            EmbedderBackend::Local(_) => self.embed_single(&formatted),
            EmbedderBackend::Ollama(remote) => remote.embed(&formatted),
        }
    }
    ...
    pub fn embed_query_batch_progress(
        ...
        if let EmbedderBackend::Ollama(remote) = &self.backend {
            ...
            return queries
                ...
                .map(|(i, q)| {
                    ...
                    let result = remote.embed(&formatted);
...
fn profile_for_model_path(model_path: &Path) -> EmbeddingProfile {
    let name = model_path
        ...
        .unwrap_or_default();
    profile_for_remote_model_name(&name)
}
...
impl OllamaEmbedder {
    ...
    fn embed_request(remote: &OllamaRemote, input: &str) -> Result<Vec<f32>> {
        let response: serde_json::Value = ureq::post(&format!("{}/api/embed", remote.base_url))
