...
impl Combined {
    ...
    pub fn try_load_default() -> Result<Option<Self>> {
        ...
        let (env_key_name, env_key): (&str, &[&str]) =
            if std::env::var_os(env::COMBINED_MODEL).is_some() {
                (env::COMBINED_MODEL, &[env::COMBINED_MODEL])
            } else if std::env::var_os(env::QWEN_MODEL).is_some() {
                eprintln!("ir: IR_QWEN_MODEL is deprecated — use IR_COMBINED_MODEL instead");
                (env::QWEN_MODEL, &[env::QWEN_MODEL])
            } else {
                return Ok(None);
            };

        // Check for ollama:// before local file resolution.
        let raw = std::env::var(env_key_name)
            .map_err(|e| Error::Other(format!("read combined env: {e}")))?;
    ...
    pub fn expand(&self, query: &str) -> Result<Vec<SubQuery>> {
        match &self.backend {
            ...
            CombinedBackend::Ollama(ollama) => ollama.expand(query),
        }
    }
    ...
    fn expand_local(&self, query: &str) -> Result<Vec<SubQuery>> {
        let local = match &self.backend {
            ...
            CombinedBackend::Ollama(_) => unreachable!(),
        };
...
impl Scorer for Combined {
    ...
    fn score_batch(&self, query: &str, docs: &[&str]) -> Result<Vec<f64>> {
        match &self.backend {
            ...
            CombinedBackend::Ollama(ollama) => ollama.score_batch(query, docs),
        }
    }
}
